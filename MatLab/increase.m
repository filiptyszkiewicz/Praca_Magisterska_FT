...
SERVO_MIN_DEG    = 0;
SERVO_MAX_DEG    = 180;
SERVO_IDLE_DEG   = 30;   % start deg
SERVO_MID_DEG    = 50;   % deg after save
TOTAL_T_S        = 60;   % time
HTTP_MIN_DT      = 0.10; 
lastHttpTic      = tic;
...
if ESP_ENABLED
    try
        ...
        lastHttpTic = tic;
        fprintf("SERVO start -> 0°\n");
    catch
        warning('ESP: error deg 0.');
    end
end
updateTitle();
...
while true
        ...
        switch ch
            ...
            case 'z'  % set 30 
                if ESP_ENABLED && toc(lastHttpTic) >= HTTP_MIN_DT
                    esp_servo('set', SERVO_IDLE_DEG, ESP_BASE);
                    servo_deg_current = SERVO_IDLE_DEG;
                    lastHttpTic = tic;
                    fprintf("SERVO -> %d° (idle)\n", SERVO_IDLE_DEG);
                end
    ...
    if ESP_ENABLED && profile_active
        ...
        % 20–60 s : 180°
        if t <= 5
            target_deg = SERVO_IDLE_DEG;            % 30°
        elseif t <= 10
            target_deg = SERVO_MAX_DEG;             % 180°
        elseif t <= 20
            tau = (t - 10) / 10;                   
            target_deg = SERVO_MAX_DEG + (SERVO_MID_DEG - SERVO_MAX_DEG) * tau; 
        else
            target_deg = SERVO_MAX_DEG;         
        end

        target_deg = round(target_deg);
        target_deg = min(SERVO_MAX_DEG, max(SERVO_MIN_DEG, target_deg));
