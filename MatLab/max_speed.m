...
updateTitle = @() set(titleBox,'String', sprintf( ...
    'Servo: %3d° | Log: %s | Post-move: %s', ...
    round(servo_deg_current), ...
    ternary(save_enable,'ON','OFF'), ...
    ternary(post_log_move_pending,'ON','OFF')));
        ...
        switch ch
            ...
            case 's'  % START: 180°
                ...
                if ESP_ENABLED
                    ...
                    fprintf("LOG START: serwo = %d°, after save %d°\n", ...
                        SERVO_MAX_DEG, SERVO_AFTER_LOG);
                end
            ...
            case 'd'  % stop save
                ...
                allow_save_start = 1;
                post_log_move_pending = false;
                fprintf("STOP: zapis zatrzymany");
        end
        set(gcf,'currentcharacter',char(0));
    end

    if ESP_ENABLED && post_log_move_pending && (save_enable == 0)
        if toc(lastHttpTic) >= HTTP_MIN_DT
            esp_servo('set', SERVO_AFTER_LOG, ESP_BASE);
            servo_deg_current     = SERVO_AFTER_LOG;
            post_log_move_pending = false;
            lastHttpTic = tic;
            fprintf("SERVO after logu -> %d°\n", SERVO_AFTER_LOG);
        end
    end
