...
while true
            ...
            case 'z'  % START:  set 30° 
                ...
                if ESP_ENABLED
                    if toc(lastHttpTic) >= HTTP_MIN_DT
                        esp_servo('set', SERVO_START_DEG, ESP_BASE);
                        servo_deg_current = SERVO_START_DEG;
                        lastHttpTic = tic;
                        pause(0.1);
                    end
                    profile_active   = true;
    ...
    if ESP_ENABLED && profile_active
        ...
        if t <= 5
            target_deg = 30;             % 0–5 s: 30°
        elseif t <= 10
            target_deg = 80;             % 5–10 s: 80°
        elseif t <= 20
            target_deg = 130;            % 10–20 s: 130°
        elseif t <= 30
            target_deg = 180;            % 20–30 s: 180°
        elseif t <= 45
            tau = (t - 30) / 15;        
            target_deg = 180 + (100 - 180) * tau;
        elseif t <= 50
            target_deg = 100;
        else
            tau = (t - 50) / 10;        
            target_deg = 100 + (180 - 100) * tau;
        end
